...
The Klima x402 Endpoint is co-developed by [Carbonmark](https://www.carbonmark.com/). It exposes carbon retirement on the **Base** blockchain through the Klima Protocol Retirement Aggregator as plain HTTP calls: it discovers carbon liquidity, returns live price quotes, hands back unsigned `[approve, retire]` calldata, and resolves the public Carbonmark certificate once the transaction confirms.

The endpoint is built for the [x402](https://www.x402.org/) agent-payments ecosystem and plugs directly into [Base MCP](https://docs.base.org/ai-agents/quickstart), so an agent can read the catalog, prepare a retirement, and submit it through the user's Base Account wallet in a single approval.
...
| Path                      | You provide                            | Gas                       | Wallet requirement                 |
| ------------------------- | -------------------------------------- | ------------------------- | ---------------------------------- |
| Build-your-own (free GET) | Your own transaction submission        | You pay gas               | A Base Account (e.g. via Base MCP) |
| Paid relay (gasless)      | One signed EIP-712 token authorization | A Klima executor pays gas | Any third-party wallet or agent    |
...
Reads never move funds. The four action endpoints accept **GET or POST**; a POST sends `{"action":"…", …params}` to `/api`.

| Method     | Path                     | Purpose                                                       |
| ---------- | ------------------------ | ------------------------------------------------------------- |
| GET / POST | `/discover`              | List retirable carbon classes, credits, and reference prices. |
| GET / POST | `/quote`                 | Live on-chain price for a tonnage in USDC or kVCM.            |
| GET / POST | `/prepare/retire`        | Unsigned `approve` + `retire` batch for atomic submission.    |
| GET / POST | `/certificate`           | Resolve Carbonmark certificate URL(s) after confirmation.     |
| GET        | `/.well-known/x402.json` | Agent-discovery manifest for x402 directories.                |
...
1. Review the [Endpoint basics](https://claude.ai/chat/96d82f18-1f30-4390-b816-4de1aa47f0e8#endpoint-basics) above and confirm you are targeting Base mainnet (`chainId=8453`).
...
\{% content-ref url="./retire-carbon-with-x402.md" %\} [Retire carbon with x402](https://claude.ai/chat/retire-carbon-with-x402.md) \{% endcontent-ref %\}

\{% content-ref url="./gasless-retirement-paid-relay.md" %\} [Gasless retirement (paid relay)](https://claude.ai/chat/gasless-retirement-paid-relay.md) \{% endcontent-ref %\}

\{% content-ref url="./use-x402-from-an-ai-agent.md" %\} [Use x402 from an AI agent](https://claude.ai/chat/use-x402-from-an-ai-agent.md) \{% endcontent-ref %\}

\{% content-ref url="./x402-reference.md" %\} [x402 reference](https://claude.ai/chat/x402-reference.md) \{% endcontent-ref %\}

\{% hint style="warning" %\} **Retirement is irreversible.** A confirmed retirement permanently burns the carbon credit — there is no undo, refund, or resale once the transaction confirms. Always review the quoted tonnes, price, and fee before approving. \{% endhint %\}
